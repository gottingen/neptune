...
export default {
  ...
  methods: {
    appRemove(endpoint) {
        axios.post('http://' + endpoint + '/ea/discovery/app/remove', {
    ...
    zoneRemove(endpoint) {
      axios.post('http://' + endpoint + '/ea/discovery/zone/remove', {
        app_name: this.appName,
        ...
        if(this.server.code === 0) {
          this.$notify({
            message: '删除成功！' + this.server.msg,
            type: 'success',
          });
        } else {
          this.$notify({
            message: '删除失败！' + this.server.msg,
            type: 'warning',
          });
        }
      }).catch(error => {
  ...
  <div>
    <p>
      ...
      <el-input type="text" v-model.lazy.trim="appName" size="small" placeholder="kumo" style="width: 40%"
                clearable></el-input>
      <el-button type="success" size="small" @click="appRemove(serverEndpoint)">删除应用</el-button>
    </p>
    ...
    <p>
      ...
      <el-button type="success" size="small" @click="zoneRemove(serverEndpoint)">删除集群</el-button>
    </p>
    <p>
      返回值 ： {{ server.code }}
      返回信息： {{ server.msg }}
